...
import { Eye, Globe, Link as LinkIcon, Calendar, Edit3, AlertCircle, Check, LayoutTemplate, Settings } from "lucide-react";
...
function StatIcon({ type }) {
  const icons = {
    views: { icon: <Eye size={20} />, bg: "linear-gradient(135deg,#6d5cff,#8b3dff)", glow: "rgba(109, 92, 255, 0.35)" },
    status: { icon: <Globe size={20} />, bg: "linear-gradient(135deg,#20c997,#11995e)", glow: "rgba(32, 201, 151, 0.35)" },
    slug: { icon: <LinkIcon size={20} />, bg: "linear-gradient(135deg,#4f7cff,#3455ff)", glow: "rgba(79, 124, 255, 0.35)" },
    date: { icon: <Calendar size={20} />, bg: "linear-gradient(135deg,#f2c94c,#c8910c)", glow: "rgba(242, 201, 76, 0.35)" },
  };
  ...
  return (
    <span
      style={{
        ...
        fontSize: 20,
        boxShadow: `0 8px 24px ${icons[type].glow}`,
        color: "#fff",
      }}
    >
      {icons[type].icon}
...
>
      <div className="editor-header" style={{ marginBottom: 18 }}>
        ...
        >
          <div>
            <h1 className="editor-title" style={{ marginBottom: 6, fontSize: 36, letterSpacing: "-0.5px" }}>
              Welcome back,{" "}
              <span style={{ color: "var(--brand-light)" }}>
                {user?.name?.split(" ")[0] || portfolio?.personalInfo?.fullName?.split(" ")[0] || "there"}
              </span>
            </h1>
            <p className="editor-subtitle" style={{ fontSize: 16 }}>
      ...
      {!portfolio ? (
        ...
        <>
          ...
          >
          <div className="editor-section">
  <h2 className="editor-section-title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><LayoutTemplate size={20} /> Portfolio Preview</h2>
  ...
  />
            ...
            <div className="editor-section">
              <h2 className="editor-section-title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Settings size={20} /> Portfolio Management</h2>
